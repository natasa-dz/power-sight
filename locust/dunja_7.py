...
class HouseholdReceiptsUser(HttpUser):
    ...
    @task
    def get_all_receipts_for_household(self):
        """Test the get all receipts for household endpoint."""
        if not self.token:
            return
        
        # Generate random household ID between 1 and 5000
        household_id = random.randint(1, 5000)
        ...
        self.client.get(
            f"/api/receipts/all-for-household/{household_id}",
            headers=headers,
            name="/api/receipts/all-for-household/{householdId}"
        )